...
Every rupee figure (invoice lines, commission, worker payout, tax, and totals)
must flow through compute_invoice(). Nothing else in the codebase should
multiply or round money.
...
- commission         = a percentage of the BASE (service charges only by
                       default; set commission_applies_to_material=True to
                       include materials). Commission is the ONLY deduction.
- tax                = tax_rate applied to gross_total (0.0 unless enabled).
- net_amount         = gross_total + tax   -> what the customer pays.
- worker_payout      = net_amount - commission -> ALWAYS reconciles exactly
                       against the other line items.
"""
...
def compute_invoice(
    ...
    worker_payout = (net_amount - commission_amount).quantize(PAISE, rounding=ROUND_HALF_UP)
